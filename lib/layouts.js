...
function timestampLevel(loggingEvent, colour) {
  return colorize(
    util.format(
      '[%s] [%s] ',
      dateFormat.asString(loggingEvent.startTime),
...
/**
 * colouredRonFiLayout
 * same colouredLayout, but without category
 */
function colouredRonFiLayout(loggingEvent) {
  if (loggingEvent.level.toString() === "ERROR" || loggingEvent.level.toString() === "WARN" || loggingEvent.level.toString() === "FATAL") {
    return timestampLevel(loggingEvent, loggingEvent.level.colour) +  colorize(util.format(...loggingEvent.data), loggingEvent.level.colour);
  } else {
    return timestampLevel(loggingEvent, loggingEvent.level.colour) +  util.format(...loggingEvent.data);
  }
}
...
module.exports = {
  ...
  colouredRonFiLayout,
  coloredLayout: colouredRonFiLayout,
  dummyLayout,
